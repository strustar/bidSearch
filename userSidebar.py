...
def sidebar():
    ...
    with st.sidebar:
        ...
        with col[0]:
            date = st.date_input(
                ":green[검색 기간]",
                (datetime.now() - timedelta(days=day), datetime.now()),
                label_visibility="collapsed",
            )
        with col[1]:
        ...
        st.markdown("---")
        st.markdown("## :orange[2. 공고명 검색 키워드]")
        col = st.columns(2)
        with col[0]:
        ...
        st.write(
            f"###### :blue[*가격 범위 : {price_range[0]/1e2:.2f} ~ {price_range[1]/1e2:.2f}억원]"
        )
        price_range = tuple(x * 1e6 for x in price_range)

        st.markdown("---")
        st.markdown("## :orange[4. 공고기관 검색 키워드]")
        col = st.columns(2)
        with col[0]:
            org_keyword = st.text_input(
                ":green[✨ 공고기관 키워드] (공백으로 구분)", value="환경 국토 지방"
            ).split()
        with col[1]:
            org_condition = st.radio(
                ":green[✨ 검색 조건을 선택하세요.]",
                ["and", "or"],
                index=1,
                horizontal=True,
