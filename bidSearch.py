...
def search_data(url, category):
    api_key = "Z3OWT7mdBJDkapPshE+8NZGPKi6iwPpt0Bdl1yGX3+mOZ33gYR32YxITUm8fKq3cn2CJGvEHm5n9OUStHImzoQ=="  # Deconding
    # api_key = 'Z3OWT7mdBJDkapPshE%2B8NZGPKi6iwPpt0Bdl1yGX3%2BmOZ33gYR32YxITUm8fKq3cn2CJGvEHm5n9OUStHImzoQ%3D%3D'  # Encoding
    params = {
        "serviceKey": api_key,
    ...
    def get_all_data():
        all_items = []
        response = requests.get(url, params=params)  # , timeout=60  60초
    ...
    try:
        ...
        if items:
            ...
            if exclude_cancel:
                df = df[
                    ~df["ntceKindNm"].str.contains("취소", na=False)
                ]  # 공고명에 '취소'가 포함되지 않은 건만 필터링

            # 2. 공고명 검색 키워드 : 공고명 필터링
            if name_keyword:
                ...
                if name_condition == "and":
                    ...
                    pattern = "|".join(map(re.escape, name_keyword))
            ...
            if org_keyword:
                # 리스트 컴프리헨션과 numpy를 활용한 최적화된 필터링
                if org_condition == "and":
                    # AND 조건: all() 사용하여 모든 키워드 포함 확인
                    mask = np.logical_and.reduce(
                        [
                            df["ntceInsttNm"].str.contains(
                                keyword, case=False, na=False
                            )
                            for keyword in org_keyword
                        ]
                    )
                else:
                    # OR 조건: 정규식 패턴을 한 번에 적용
                    pattern = "|".join(map(re.escape, org_keyword))
                    mask = df["ntceInsttNm"].str.contains(pattern, case=False, na=False)
            ...
            df["수요기관"] = df.apply(
                lambda row: (
                    "-" if row["공고기관"] == row["수요기관"] else row["수요기관"]
                ),
                axis=1,
            )
            # '등록구분명' 컬럼의 값이 '자체'를 포함하면 '자체 공고건'으로 변경
            if "등록구분" in df.columns:
                df["등록구분"] = df["등록구분"].apply(
                    lambda x: "자체 공고" if "자체" in str(x) else x
                )
            ...
            df.rename(
                columns={"공고번호": "공고번호 (차수)"}, inplace=True
            )  # 컬럼 이름 변경
            ...
            create_ag_grid(df, name_keyword)
...
condition = (
    " (모두 포함된 공고명)" if name_condition == "and" else " (하나라도 포함된 공고명)"
)
condition = " (포함된 공고명)" if len(name_keyword) == 1 else condition
col = st.columns([1, 4])
with col[0]:
    view_type = st.radio(
        "💼 용역 / 🏗️ 공사",
...
with col[1]:
    st.write(
        f"###### 🔎 검색 유형 : :green[[{view_type}]]",
        f",&nbsp;&nbsp;&nbsp; 검색어 : :green[{name_keyword}] {condition}",
        f",&nbsp;&nbsp;&nbsp; 가격 범위 : :green[{price_range[0]/1e8:.2f} ~ {price_range[1]/1e8:.2f}억원]",
    )
...
service_url = "/getBidPblancListInfoServc"
# construction_url = '/getBidPblancListInfoCnstwkPPSSrch02'
construction_url = "/getBidPblancListInfoCnstwk"

if "용역" in view_type:
    url = base_url + service_url
    try:
        items, df = search_data(url, "service")
        if items is None or df is None:
            st.stop()
    except Exception as e:
        st.error(f"용역 데이터 조회 중 오류 발생: {str(e)}")
        st.stop()
    ...
    try:
        ...
        st.error(f"공사 데이터 조회 중 오류 발생: {str(e)}")
        st.stop()
...
with st.sidebar:
    t1 = time.time()
    st.write(
        f"기간내 :green[총 {len(items)}건 중]에서 조건에 맞는 :blue[{len(df)}건 검색]"
    )
    st.write(
        f"총 컬럼 수 (정보 갯수) : {len(df.columns)}개",
        f",&nbsp;&nbsp;&nbsp; 검색 시간: {t1-t0:.2f}초",
    )
